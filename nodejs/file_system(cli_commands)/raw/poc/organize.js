...
//Step1. create a directory inside the given path
//subdirectory
let input=process.argv.slice(2);
let toOrganizeDirPath=input[0];
let organizedFilesPath=path.join(toOrganizeDirPath,"organized_files");

//for creating a directory on dirPath->shows path where dir has to be created
    ...
    if(isFile==true)
    {
        ...
        copytodest(dirPath,destFolderPath);
          ...
          for(let i=0;i<childrens.length;i++)
          {
            ...
            orgFiles(path.join(dirPath,childrens[i]));
          }
        }
}
